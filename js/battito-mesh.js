(function () {
    ...
    var canvas = document.getElementById('brl-echo-mesh');
    if (!canvas) return;
    ...
    var fragmentSource = [
        ...
        '  vec3 col = u_colors[0];',
        '  for (int i = 0; i < 7; i++) {',
        '    if (float(i) < n) col = mixColour(col, u_colors[i + 1],',
        ...
        '  if (u_warp > 0.0) p += u_warp*(vec2(fbm(p*u_detail+u_seed),',
        ...
        '  if (u_grain>0.0001) col+=(grainHash(gl_FragCoord.xy+',
        '    vec2(u_seed*17.0,u_seed*31.0))-0.5)*u_grain;',
    ...
    var uniforms = {};
    ['u_scene', 'u_shape', 'u_surface', 'u_finish', 'u_transform', 'u_space', 'u_cursor'].forEach(function (name) {
        uniforms[name] = gl.getUniformLocation(program, name);
    });
    uniforms.colors = gl.getUniformLocation(program, 'u_colors[0]');
    ...
    /* The source preset targets a full screen. A wider field keeps its blobs
       legible inside this very panoramic action strip. */
    gl.uniform4f(uniforms.u_shape, 0.52, 0.64, 0.28, 0.0);
    gl.uniform4f(uniforms.u_surface, 1.82, 0.99, 0.0, 1.0);
    gl.uniform4f(uniforms.u_finish, 0.0, 0.0, 0.0, 0.07);
    gl.uniform4f(uniforms.u_transform, 1.0, 0.0, 0.12, 1.0);

    var pointer = { x: 0, y: 0, presence: 0, target: 0 };
    ...
    function render(now) {
        resize();
        pointer.presence += (pointer.target - pointer.presence) * 0.12;
        gl.uniform4f(uniforms.u_scene, canvas.width, canvas.height,
            ((now - start) / 1000) * 2.0, 3.0);
        gl.uniform4f(uniforms.u_space, 0.0, 0.0, pointer.x, pointer.y);
        gl.uniform4f(uniforms.u_cursor, pointer.presence, 2.0, 0.30, 0.62);
